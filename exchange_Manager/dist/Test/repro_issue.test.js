"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const vitest_1 = require("vitest");
const Engine_1 = require("../trade/Engine");
const fromApi_1 = require("../Types/fromApi");
// Mock RedisManager
const publishMessageMock = vitest_1.vi.fn();
vitest_1.vi.mock("../RedisManager", () => ({
    RedisManager: {
        getInstance: () => ({
            publishMessage: publishMessageMock,
            sendToApi: vitest_1.vi.fn(),
            pushMessage: vitest_1.vi.fn()
        })
    }
}));
(0, vitest_1.describe)("Engine Depth Updates", () => {
    (0, vitest_1.it)("should publish 0 quantity when an order is fully filled", () => {
        const engine = new Engine_1.Engine();
        const market = "TATA_INR";
        const price = "1000";
        const quantity = "10";
        // 1. Place a SELL order (Maker)
        engine.process({
            message: {
                type: fromApi_1.CREATE_ORDER,
                data: {
                    market,
                    price,
                    quantity,
                    side: "sell",
                    userId: "1"
                }
            },
            clientId: "1"
        });
        // Clear previous calls
        publishMessageMock.mockClear();
        // 2. Place a BUY order (Taker) that fully consumes the SELL order
        engine.process({
            message: {
                type: fromApi_1.CREATE_ORDER,
                data: {
                    market,
                    price,
                    quantity, // Same quantity to fully fill
                    side: "buy",
                    userId: "2"
                }
            },
            clientId: "2"
        });
        // 3. Check the depth update
        // We expect a message to 'depth@TATA_INR'
        // The data should contain 'a' (asks) with [price, "0"]
        const depthCalls = publishMessageMock.mock.calls.filter(call => call[0] === `depth@${market}`);
        (0, vitest_1.expect)(depthCalls.length).toBeGreaterThan(0);
        const lastDepthCall = depthCalls[depthCalls.length - 1];
        const payload = lastDepthCall[1];
        const data = payload.data;
        console.log("Last Depth Update Data:", JSON.stringify(data, null, 2));
        // Expect asks to contain [price, "0"]
        const askUpdate = data.a.find((x) => x[0] === price);
        (0, vitest_1.expect)(askUpdate).toBeDefined();
        (0, vitest_1.expect)(askUpdate[1]).toBe("0");
    });
});
